...
const autoAddTooltip = function (event) {
  ...
  if (element instanceof Element && element !== lastMouseOverElement) {
    ...
    let innerRun = false;
    const inner = (content, el, { dataTitle, dataAvoid, dataAvoidBackup }) => {
      innerRun = true;
...
export const startAutoTooltip = () => {
  document.addEventListener("mouseover", autoAddTooltip);
};

export const stopAutoTooltip = () => {
  document.removeEventListener("mouseover", autoAddTooltip);
};
